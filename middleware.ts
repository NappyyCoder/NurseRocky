...
export const config = {
  matcher: [
    // /sign-out clears stale Clerk cookies without running the handshake
    "/((?!_next|sign-out|[^?]*\\.(?:html?|css|js(?!on)|jpe?g|webp|png|gif|svg|ttf|woff2?|ico|csv|docx?|xlsx?|zip|webmanifest)).*)",
    "/(api|trpc)(.*)",
  ],
};
